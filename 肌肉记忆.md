...
Iterative way is good, because it needs much lees space of stack, thus avoiding stack overflow.
...
#### Solution 2: iterative (stack pop栈顶并打印，右进去，左进去)
...
#### Solution 2: iterative (stack + helper)

1. when helper is not null, we should traverse the subtree, so we push helper and we go left
    * helper 意义1: 下一个被遍历的节点
2. when helper is null, which means the left subtree of the root is finished, the root is the top element in the stack. We can print the top, and let helper = top.right. (traverse the left subtree first, then top, then right subtree)
    * helper 意义2: `helper == null` 意味着栈顶元素是一个左子树都被打印完毕的根
3. do 1 and 2 until helper is null and there is no nodes left in the stack
    * 结束时 `helper == null && stack.isEmpty()`
...
##### Solution 2.0: 奇技淫巧, (left, right, root) -> reverse -> (root, right, left)
...
##### Solution 2.1: 三顾茅庐诸葛亮，第三次看见才打印 (stack + previous + current)
...
- previous 和 current 记录从哪里来的，记得维护 previous
    1. if previous is null: going down (left subtree has priority)
    2. if previous is current's parent: going down (left subtree has priority)
    3. if `previous == current.left`: left subtree finished, going current.right
    4. if `previous == current.right`: right subtree finished, pop current and print, going up
...
class Solution {
    private BigInteger nPk(int n, int k) {
        int mod = 1_000_000_007;
        BigInteger result = BigInteger.ONE;
        for (int i = 1; i <= k; i++) {
            result = result.multiply(BigInteger.valueOf(n - (i - 1)));
        }
        return result.mod(BigInteger.valueOf(mod));
    }
}
```
...
class UnionFind {
    constructor(n) {
        this.parent = [], this.size = [], this.count = n;
        for (let i = 0; i <= n; i++) {
            this.parent[i] = i, this.size[i] = 1;
        }
    }
